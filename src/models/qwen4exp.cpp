...
void llama_model_qwen4exp::load_arch_hparams(llama_model_loader & ml) {
    ...
    // Upstream reads this into a per-layer array (n_ff_exp_arr); this tree has only the
    // scalar. Equivalent while every block shares one expert FFN width, which holds for
    // Qwen3.8-Flash-Next - revisit if a model ships an MTP block of a different width.
...
void llama_model_qwen4exp::load_arch_tensors(llama_model_loader & ml) {
    ...
    for (int il = 0; il < (int) hparams.n_layer_all; ++il) {
        ...
        // scalar here, per-layer array upstream - see load_arch_hparams
